        ...
        {foreach from=$nodes item=node}
          <li data-depth="{$depth}">
            {if $depth===0}
              ...
              {if $node.children}
                <div class="navbar-toggler collapse-icons" aria-expanded="true" data-toggle="collapse" data-target="#exCollapsingNavbar{$node.id}">
              ...
              {/if}
            {else}
              <a class="category-sub-link" href="{$node.link}">{$node.name} [{$node.productsCount}]</a>
              {if $node.children}
